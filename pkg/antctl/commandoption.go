...
const (
	...
	// 	}
	// If the field is annotated with antctl:"key", the framework assumes this field could be used to retrieve a unique
	// Response, thus the framework will generate corresponding arg options to the cobra.Command.
	TagKey = "antctl"
...
// Validate checks if the CommandOption is valid.
...
func (co *CommandOption) format(cmd *cobra.Command, r io.Reader, w io.Writer, single bool) (err error) {
	var obj interface{}
	if co.AddonTransform != nil { // Use AddonTransform to decode the data, if it exists.
	...
	if co.AddonTransform == nil { // Decode the data if there is no AddonTransform.
...
func (co *CommandOption) applyToCommand(cmd *cobra.Command, client *Client) {
	...
	// If no example is provided, the framework will generate the vanilla flavor.
	if len(co.Example) == 0 {
