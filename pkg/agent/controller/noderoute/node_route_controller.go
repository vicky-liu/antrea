...
func (c *Controller) deleteNodeRoute(nodeName string) error {
	...
	if c.ipsecPSK != "" {
		// Delete the tuennl port of this Node.
	}
	return nil
}
...
func (c *Controller) createIPSecTunnelPort(nodeName string, nodeIP net.IP) (int32, error) {
	...
	if ok {
		...
		if portUUID, err := c.ovsBridgeClient.CreateTunnelPortExt(
			portName,
			c.tunnelType,
	...
	if ofPort, err := c.ovsBridgeClient.GetOFPort(interfaceConfig.InterfaceName); err != nil {
...
func ParseTunnelInterfaceConfig(
	...
	if psk != "" {
		return interfacestore.NewIPSecTunnelInterface(
			portData.Name,
		...
		return interfacestore.NewTunnelInterface(portData.Name, ovsconfig.TunnelType(portData.IFType))
	}
}
